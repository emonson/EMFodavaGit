...
scale = 4;
initNum = 26;       % Original number of points picked
incNum = 1;     % How many each time when picking uncertain points
allowedDrop = 0.0025;    % How far overall accuracy allowed to drop before points are thrown out.
...
numLeaveOut = 1;   % Not used if leaveOutFracOfTotal = true;
leaveOutFrac = 0.1;
...
figure; 
subplot(3,1,1);
plot(ptCount,sc,'.-','Color',[0.2 0.2 0.5],'LineWidth',2,'MarkerSize',20);
hold on;
plot(ptCount,pc,'-','Color',[0.7 0.7 0.9]);
