...
m = 5;
n_labels = length(imgOpts.Labels);
...
% NOTE: Only using one group for now
rr = 1;

X_train = X(:,groups ~= rr);
X_test = X(:,groups == rr);
labels_train = imgOpts.Labels(groups ~= rr);
labels_test = imgOpts.Labels(groups == rr);

% Getting rid of imgOpts.Labels to make sure I don't screw up train/test...
imgOpts = rmfield(imgOpts, 'Labels');
imgOpts.Labels_train = labels_train;
imgOpts.Labels_test = labels_test;

% Getting rid of X to make sure train/test code isn't screwed up...
clear('X');
...
if (length(root_idx) > 1)
    ...
    % This routine calculates errors for the children of the current node
    % so we need to first calculate the root node error
    [total_errors, std_errors] = gwt_single_node_lda_traintest( GWT, Data_train, Data_test, imgOpts, root_idx, COMBINED );
    ...
    while (~node_idxs.isEmpty())
        ...
        for current_child_idx = current_children_idxs,
            ...
            % Calculate the error on the current child
            [total_errors, std_errors] = gwt_single_node_lda_traintest( GWT, Data_train, Data_test, imgOpts, current_child_idx, COMBINED );
...
h_use_self_bool = ee_h >= USE_SELF;
plot(x(h_use_self_bool), y(h_use_self_bool), '.', 'MarkerSize', 14, 'Color', [0.6 0.0 0.0]);
...
text(x(finite_values,1), y(finite_values,1), besterr_strings(finite_values), ...
    'VerticalAlignment','top','HorizontalAlignment','right','Color',[0.2 0.6 0.2])
% Node cp index
% text(x(:,1), y(:,1), cp_idx_strings, ...
%     'VerticalAlignment','bottom','HorizontalAlignment','left','Color',[0.2 0.2 0.6])
% Holdout error
text(x(finite_holdout_vals,1), y(finite_holdout_vals,1), holderr_strings(finite_holdout_vals), ...
    'VerticalAlignment','bottom','HorizontalAlignment','left','Color',[0.2 0.2 0.2])
% Best Holdout child error
text(x(finite_holdout_vals,1), y(finite_holdout_vals,1), bestholderr_strings(finite_holdout_vals), ...
    'VerticalAlignment','top','HorizontalAlignment','left','Color',[0.2 0.2 0.6])
