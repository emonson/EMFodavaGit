...
% Add noise to data
pNoisyRealizations = 20;
pNoiseLevel = (20).^2; %(51:-5:1).^2;
idx = 1;
clear X Y
...
% Solid vs. Liquid
%allLabels = material_names_classes(:,3);                            % Liquid vs. solid
allLabels = material_names_classes(:,2);                            % dangerous, not dangerous, falmmable, oxidizer, BDPO,...
[uniqueLabels,~,Labels] = unique(allLabels(Y));
Labels = Labels';


%% Generate data and choose parameters for GWT
GWTopts = struct('knn',100,'knnAutotune',20,'ManifoldDimension',0,'threshold0',0,'GWTversion',0,'precision',1e-1);
