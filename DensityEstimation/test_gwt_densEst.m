...
for j = 1:nScales,
    ...
    [LogL(m,j), LogLPerPlan{m,j}, ~] = EvalLLFromMultiplePlaneDens...
        ( DensEst_GMRA{j}, X_val, struct('mode', 'GMRA', 'GMRA', gMRA, 'DataGWT', DataGWT_val) );

    for k = 1:length(LogLPerPlan{m,j}),
        if DensEst_GMRA{j}.child_idx(k) > 0, % nonleaf node
            matF(j, DataGWT_val.PointsInNet{DensEst_GMRA{j}.child_idx(k)}) = LogLPerPlan{m,j}{k};
        else
            matF(j, DataGWT_val.PointsInNet{DensEst_GMRA{j}.cp_idx(k)})    = LogLPerPlan{m,j}{k};
        end;
    end;
...
matF = matF(:, idx_sort);
 DrawGWTWavSubFcn( gMRA, matF, 'log-Likelihood');
